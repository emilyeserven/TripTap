...
|---|---|---|---|
| Sentences (bank / mine / practice) | `sentences.*` (`index` with `?view=`, `new`, `import`, `$id`, `$id.edit`; `my-sentences.*` and `practice.*` are redirect stubs) | `sentences.ts` (+ `sentence-vocab`, `furigana`) | **One `sentences` table for every sentence**, discriminated by `kind` (`bank` reference examples · `mine` learner-written with the needs-correction flow · `practice` study cards with passes and breakdowns). All facets are nullable columns on the one row; `derived_from_id` is the self-FK lineage (practice card ← bank sentence, mine ← practice output, promoted bank row ← mine). One API (`/api/sentences?kind=…` + lineage/state filters), one wire type (`types/src/sentence.ts` — which also records that this merge deliberately reversed an earlier decision against it). The list page renders per-kind views behind tabs; detail/edit pages branch on `kind` (mine → corrector, practice → 6-tab autosaving worksheet). |
...
| Grammar | `grammar.tsx` | *(read AI-lesson content; no own route)* | Cross-lesson view over AI-lesson data (redirects to `/grammar-notes`). |
| Culture Notes | `culture.*` | `culture-notes.ts`, `culture-topics.ts` | Unified Culture page: user-authored bilingual notes (optional EN/JA bodies, at least one — service-enforced) alongside AI-lesson culture cards. Both are tagged by id (jsonb, no FK) into the flat `culture_topics` taxonomy (manager at `/culture/topics`); AI cards get tagged via `PATCH /api/ai-lesson-culture/:id`. |
| Vocab | `vocabulary.tsx` (`vocab.tsx` redirects) | `vocab.ts` | Standalone vocab bank, unified with AI-lesson vocab. |
| Kanji | `kanji.index.tsx`, `kanji.$char.tsx` | `kanji.ts` → `services/kanji/` | Per-character view of the corpus. **Derived on read** like XP — scanning `sentences` (all kinds; for mine-kind rows the correction when there is one), `vocab`, `dialogues` (parsed `lines`, *not* `script`, or speaker names dominate) and `writings` yields per-kanji counts, first/last seen, and every item containing it; the only stored state is the learner's `new`/`learning`/`known` mark in `kanji_status`. The character predicates live in `types/src/kanji-chars.ts`: `HAS_KANJI_RE`/`hasKanji` is the furigana gate (includes 々, shared with `SentenceForm`/`FuriganaEditor`/`SentenceCardTools`), while `isKanji`/`extractKanji` index kanji proper — `Script=Han` minus the ideographic marks 々〆〇〻, which carry Han but aren't characters to learn. |
...
| Dialogues | `dialogues.*` | `dialogues.ts` | Multi-speaker scripts (`話者：セリフ`, `：` or `:`) rendered as chat bubbles (`ui/chat-bubble.tsx`). The parser + reconciler live in `types/src/dialogue.ts` so the API and the live form preview agree; `私`/`Me` (plus per-dialogue `selfSpeakers`) render on the right. Speakers can be hidden for practice (view-only); a line's `hint` is shown in place of the words while hidden, so the learner still knows what the turn is for. A self-written dialogue can be flagged `countsTowardXp` to earn XP in a chosen `learningArea`. |
...
| Sources | `sources.*` | `sources.ts` | Source taxonomy (book/show/article) referenced by sentences, vocab, captures. Nests to any depth via `parentId` (magazine → issue → the page a few captures came from), and every tier keeps its own `url`. The traversals (`sourceAncestors`, `sourceChildren`, `sourceDescendants`, `flattenSourceTree`, `wouldCreateSourceCycle`) live in `types/src/source-tree.ts` so the API's cycle guard and the client's trees/pickers agree; deleting a parent clears the link rather than cascading. Filtering by a source anywhere in the client matches its whole subtree (`client/src/lib/source-options.ts`). |
...
| Renshuu examples | *(Renshuu tab in the drill-mistake picker; `RenshuuImportDialog` on the Sentences bank view)* | `renshuu.ts` → `services/renshuu/` | Read-only proxy to renshuu.org's example-sentence bank (`/v1/reibun/search`, Bearer key). Renshuu over-kanjifies, so `orthography.ts` rewrites conventionally-kana words (`此の`→`この`, `為る`→`する`) and furigana is generated on the normalized text; import single results or in bulk into the bank. |
| Handwriting | *(pencil toggle inside `WordLookup`; `HandwritingPad.tsx` + `lib/handwriting.ts`)* | `handwriting.ts` → `services/handwriting/` | Draw-to-lookup for the dictionary: canvas strokes → proxy → ranked candidate characters, appended to the search box one at a time so a word can be built up. Wire types are provider-neutral (`HandwritingStroke`); the upstream ink encoding stays in the service. |
...
| `HANDWRITING_API_URL` | middleware | Optional override for the handwriting-recognition base URL (default `https://inputtools.google.com`; the service POSTs to `/request?itc=ja-t-i0-handwrit`). Backs the draw-a-character pad on the dictionary lookup. No auth; **undocumented/unofficial**, so the response is parsed defensively and an unknown shape degrades to "no candidates". |

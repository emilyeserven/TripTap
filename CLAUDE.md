# TripTap — Architecture & Conventions

## Project summary

A full-stack TypeScript monorepo for tracking trips. Built with pnpm workspaces, mirroring the
...
- **types** (`@triptap/types`) — shared TypeScript definitions; builds to `dist`.
- **middleware** (`@triptap/middleware`) — Fastify API. Layered `src/`: `db/` (Drizzle schema +
  client + seed), `routes/`, `services/`, `utils/`, `tests/`, `app.ts`, `index.ts`.
- **client** (`@triptap/client`) — React frontend. `src/`: `routes/` (file-based), `components/`,
  `hooks/`, `lib/`, `stores/`, `test-utils/`.
- **gateway** (`@triptap/gateway`) — Fastify reverse proxy, the production entrypoint (`server.js`).
...
Package-scoped commands use `pnpm --filter=@triptap/<name>`.
...
- `packages/client/src/routeTree.gen.ts` — regenerate with `pnpm --filter=@triptap/client routeTree`

...
| Dialogues | `dialogues.*` | `dialogues.ts` | Multi-speaker scripts (`話者：セリフ`, `：` or `:`) rendered as chat bubbles (`ui/chat-bubble.tsx`). The parser + reconciler live in `types/src/dialogue.ts` so the API and the live form preview agree; `私`/`Me` (plus per-dialogue `selfSpeakers`) render on the right. Speakers can be hidden for practice (view-only). A self-written dialogue can be flagged `countsTowardXp` to earn XP in a chosen `learningArea`. |

...
/**
 * Build a create input from a Renshuu result. The text is already normalized toward the common
 * written form server-side; the `renshuu` tag plus a source note keep imported rows identifiable
 * whatever kind the learner picks.
 */
function toInput(example: RenshuuExampleSentence): CreateSentenceInput {
  return {
    text: example.text,
 ...
 * sentences as the kind they pick. The server-generated furigana previews here; the import
 * regenerates it (with vocab overrides). Parallels {@link ./TatoebaImportDialog}.
 */
